...
class PDF {
	...
	private static function _getCPU()
	{
		if(self::$cpu=='')
		{
			if(`uname -a | grep -i x86_64` != '') self::$cpu='amd64';
			else self::$cpu='i386';
//			elsef(`grep -i intel /proc/cpuinfo`!='') self::$cpu='i386';
//			else throw new Exception('WKPDF couldn\'t determine CPU ("'.`grep -i vendor_id /proc/cpuinfo`.'").');
	...
	public function __construct()
	{
		...
		$static_path = APPPATH . 'third_party' . DIRECTORY_SEPARATOR . 'wkhtmltopdf' . DIRECTORY_SEPARATOR . 'bin' . DIRECTORY_SEPARATOR;
		$this->cmd = $static_path . 'wkhtmltopdf-'.self::_getCPU();
		if(!file_exists($this->cmd)) throw new Exception('WKPDF static executable "'.htmlspecialchars($this->cmd,ENT_QUOTES).'" was not found.');
		do {
			$this->tmp = realpath(APPPATH . '../') . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . mt_rand() . '.html';
